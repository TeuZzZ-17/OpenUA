...
static mat3x3 yw_ImpactScarBasis(const vec3d &normal)
{
    vec3d n = normal;
    if ( n.normalise() <= 0.001 )
        n = vec3d(0.0, -1.0, 0.0);

    vec3d ref = fabs(n.y) < 0.9 ? vec3d(0.0, 1.0, 0.0) : vec3d(1.0, 0.0, 0.0);
    vec3d tangent = ref * n;
    if ( tangent.normalise() <= 0.001 )
        tangent = vec3d(1.0, 0.0, 0.0);

    vec3d bitangent = n * tangent;
    if ( bitangent.normalise() <= 0.001 )
        bitangent = vec3d(0.0, 0.0, 1.0);

    return mat3x3(tangent.x,   bitangent.x,   n.x,
                  tangent.y,   bitangent.y,   n.y,
                  tangent.z,   bitangent.z,   n.z);
}

static float yw_ImpactScarClamp(float v, float minv, float maxv)
{
    if ( v < minv )
        return minv;
    if ( v > maxv )
...
static float yw_ImpactScarSmoothStep(float edge0, float edge1, float x)
{
    if ( edge1 <= edge0 )
        return x >= edge1 ? 1.0f : 0.0f;

    float t = yw_ImpactScarClamp((x - edge0) / (edge1 - edge0), 0.0f, 1.0f);
    return t * t * (3.0f - 2.0f * t);
}

static void yw_ImpactScarBasisVectors(const vec3d &normal, vec3d *tangent, vec3d *bitangent, vec3d *outNormal)
{
    vec3d n = normal;
    if ( n.normalise() <= 0.001 )
        n = vec3d(0.0, -1.0, 0.0);
...
static bool yw_ImpactScarRaycast(NC_STACK_ypaworld *yw, const vec3d &start, const vec3d &vect, ypaworld_arg136 *out)
{
    if ( !yw )
        return false;
...
static vec3d yw_ImpactScarSampleNormal(const ypaworld_arg136 &ray, const vec3d &preferredNormal)
{
    vec3d n = preferredNormal;

    if ( ray.skel && ray.polyID >= 0 && (size_t)ray.polyID < ray.skel->polygons.size() )
        n = ray.skel->polygons[ray.polyID].Normal();
    ...
    vec3d preferred = preferredNormal;
    if ( preferred.normalise() > 0.001 && n.dot(preferred) < 0.0 )
        n = -n;

    return n;
}

static bool yw_ImpactScarSampleTerrainPoint(NC_STACK_ypaworld *yw, const NC_STACK_ypaworld::TImpactScar *scar,
                                            const vec3d &xzPoint, vec3d *outPos)
{
    static const float SCAN_DISTANCE = 6000.0f;

    float maxDelta = scar->radius * 4.0f;
    if ( maxDelta < 350.0f )
        maxDelta = 350.0f;

    auto sampleAt = [&](const vec3d &p, vec3d *samplePos, vec3d *sampleNormal)
    {
        ypaworld_arg136 ray;
        vec3d start(p.x, p.y - SCAN_DISTANCE * 0.5f, p.z);
        vec3d vect(0.0, SCAN_DISTANCE, 0.0);

        if ( !yw_ImpactScarRaycast(yw, start, vect, &ray) )
            return false;

        if ( !yw->IsImpactScarTerrainHit(ray) )
            return false;

        if ( fabs(ray.isectPos.y - scar->pos.y) > maxDelta )
            return false;

        vec3d n = yw_ImpactScarSampleNormal(ray, vec3d(0.0, -1.0, 0.0));
        if ( n.y > 0.0 )
            n = -n;

        *samplePos = ray.isectPos;
        *sampleNormal = n;
        return true;
    };
    ...
    bool found = false;
    float bestScore = SCAN_DISTANCE;
    vec3d bestPos;
    vec3d bestNormal(0.0, -1.0, 0.0);

    for (const vec3d &offset : offsets)
    {
        vec3d samplePos;
        vec3d sampleNormal;
        if ( !sampleAt(xzPoint + offset, &samplePos, &sampleNormal) )
            continue;

        float delta = fabs(samplePos.y - scar->pos.y);
        float flatnessPenalty = (1.0f - yw_ImpactScarClamp(fabs(sampleNormal.y), 0.0f, 1.0f)) * 20.0f;
        float offsetPenalty = offset.length() * 0.25f;
        float score = delta + flatnessPenalty + offsetPenalty;
    ...
    *outPos = bestPos + bestNormal * 3.0;
    return true;
}

static void yw_BuildImpactScarMesh(NC_STACK_ypaworld *yw,
                                   NC_STACK_ypaworld::TImpactScar *scar,
                                   const World::TWeaponImpactScarConfig &config)
{
    GFX::TMesh &mesh = scar->mesh;
    mesh.Vertexes.clear();
    mesh.Indixes.clear();
    mesh.CoordsCache.clear();
    mesh.Mat = GFX::TRenderParams(GFX::RFLAGS_ZEROTRACY | GFX::RFLAGS_DISABLE_ZWRITE);

    float r = (float)config.color.r / 255.0f;
    float g = (float)config.color.g / 255.0f;
    float b = (float)config.color.b / 255.0f;
    float centerAlpha = (float)config.color.a / 255.0f;

    // Conforming grid decal. Each visible vertex is sampled back onto world
    // geometry; invalid points are left transparent so edges and voids clip away
    // instead of becoming large stretched triangles.
    static const int GRID = 40;

    float edgeFade = yw_ImpactScarClamp(config.edge_fade, 0.05f, 1.0f);
    float fadeStart = yw_ImpactScarClamp(1.0f - edgeFade, 0.0f, 0.95f);

    auto radialAlpha = [fadeStart](float t)
    {
        if ( t >= 1.0f )
            return 0.0f;

        float fade = yw_ImpactScarSmoothStep(fadeStart, 1.0f, t);
        return 1.0f - fade;
    };

    vec3d tangent, bitangent, n;
    yw_ImpactScarBasisVectors(scar->normal, &tangent, &bitangent, &n);
    scar->normal = n;

    float renderRadius = scar->radius;

    mesh.Vertexes.reserve((GRID + 1) * (GRID + 1));
    mesh.Indixes.reserve(GRID * GRID * 12);

    std::vector<uint8_t> validVertices((GRID + 1) * (GRID + 1), 0);
    int visibleVertices = 0;

    for (int y = 0; y <= GRID; y++)
    {
        float fy = ((float)y / (float)GRID) * 2.0f - 1.0f;
        for (int x = 0; x <= GRID; x++)
        {
            float fx = ((float)x / (float)GRID) * 2.0f - 1.0f;
            float dist = sqrt(fx * fx + fy * fy);
            float alpha = dist <= 1.0f ? centerAlpha * radialAlpha(dist) : 0.0f;

            int vertIndex = y * (GRID + 1) + x;
            vec3d sample = scar->pos;
            bool sampled = false;

            if ( alpha > 0.0f )
            {
                vec3d xzPoint(scar->pos.x + fx * renderRadius, scar->pos.y, scar->pos.z + fy * renderRadius);
                sampled = yw_ImpactScarSampleTerrainPoint(yw, scar, xzPoint, &sample);
            }
            ...
            {
                validVertices[vertIndex] = 1;
                visibleVertices++;
            }

            vec3d local = sample - scar->pos;
            mesh.Vertexes.emplace_back(vec3f((float)local.x, (float)local.y, (float)local.z),
                                       tUtV((fx + 1.0f) * 0.5f, (fy + 1.0f) * 0.5f),
                                       GFX::TGLColor(r, g, b, alpha));
        }
    }

    // If almost nothing could be projected, keep the scar invisible instead of
    // falling back to the old flat-plane smear.
    if ( visibleVertices < 12 )
    {
        mesh.Vertexes.clear();
        mesh.Indixes.clear();
        return;
    ...
    auto addTri = [&mesh](int a, int b, int c)
    {
        mesh.Indixes.push_back(a);
        mesh.Indixes.push_back(b);
        mesh.Indixes.push_back(c);

        // Keep the decal visible from both sides because world surface normals are not
        // always reliable for terrain/building collision callbacks in this legacy renderer.
        // With GL culling enabled, only one winding is normally drawn, so this does not
        // double-blend the scar in the common path.
        mesh.Indixes.push_back(a);
        mesh.Indixes.push_back(c);
        mesh.Indixes.push_back(b);
    };
    ...
    auto vertexValid = [&validVertices](int idx)
    {
        return idx >= 0 && (size_t)idx < validVertices.size() && validVertices[idx] != 0;
    };
    ...
    for (int y = 0; y < GRID; y++)
    {
        for (int x = 0; x < GRID; x++)
        {
            int v00 = gridIndex(x, y);
            int v10 = gridIndex(x + 1, y);
            int v01 = gridIndex(x, y + 1);
            int v11 = gridIndex(x + 1, y + 1);

            // Never bridge valid decal vertices through invalid/transparent points.
            // This is especially important on building edges, where a single bad
            // sample used to create long semi-transparent triangles through the scene.
            if ( vertexValid(v00) && vertexValid(v10) && vertexValid(v11) )
                addTri(v00, v10, v11);
            if ( vertexValid(v00) && vertexValid(v11) && vertexValid(v01) )
                addTri(v00, v11, v01);
        }
    }
...
bool NC_STACK_ypaworld::IsImpactScarTerrainHit(const ypaworld_arg136 &hit)
{
    if ( !hit.isect )
        return false;

    if ( hit.hitCell.x < 0 || hit.hitCell.x >= _mapSize.x ||
         hit.hitCell.y < 0 || hit.hitCell.y >= _mapSize.y )
        return false;

    const cellArea &cell = _cells(hit.hitCell);
    if ( !cell.IsGamePlaySector() ||
         cell.SectorType == 1 ||
         cell.PurposeType != cellArea::PT_NONE )
        return false;

    if ( hit.skel && hit.polyID >= 0 && (size_t)hit.polyID < hit.skel->polygons.size() )
    {
        vec3d n = hit.skel->polygons[hit.polyID].Normal();
        if ( n.normalise() > 0.001 && fabs(n.y) < 0.25f )
            return false;
    }

    // CollisionType 1 is a lego surface. On ordinary sectors that can still be
    // valid ground, but roofs and decorative building tops sit well above the
    // cell terrain height. Filler collision types 2/3/4 are kept for sector edges.
    if ( hit.hitCollisionType == 1 && fabs(hit.isectPos.y - cell.height) > 260.0f )
        return false;
...
bool NC_STACK_ypaworld::AddImpactScar(const World::TWeaponImpactScarConfig &config, const vec3d &pos, const vec3d &normal)
{
    ...
    scar.pos = pos;
    scar.normal = normal;
    if ( scar.normal.normalise() <= 0.001 )
        scar.normal = vec3d(0.0, -1.0, 0.0);
    scar.radius = scarRadius;
    scar.startTime = _timeStamp;
    scar.duration = config.duration;
    scar.fadeTime = config.fade_time;
    scar.seed = (uint32_t)(_timeStamp * 1103515245u) ^
                (uint32_t)((int)pos.x * 73856093u) ^
                (uint32_t)((int)pos.y * 19349663u) ^
                (uint32_t)((int)pos.z * 83492791u);
...
void NC_STACK_ypaworld::RenderImpactScars(baseRender_msg *arg)
{
    ...
    {
        int32_t age = _timeStamp - it->startTime;
        if ( age < 0 )
            age = 0;
        ...
        float alpha = 1.0f;
        int32_t fadeTime = it->fadeTime;
        if ( fadeTime > 0 )
        {
            int32_t fadeStart = it->duration - fadeTime;
            if ( fadeStart < 0 )
                fadeStart = 0;
            if ( age >= fadeStart )
                alpha = 1.0f - (float)(age - fadeStart) / (float)fadeTime;
        }
        ...
        // Impact scar meshes are already baked as local offsets from scar.pos.
        // Keep only translation here; rotating again would detach conforming vertices
        // from slopes/walls.
        mat4x4 world;
        world += it->pos;
        ...
        rend.Mesh = &it->mesh;
        rend.Flags = it->mesh.Mat.Flags;
        rend.Color = it->mesh.Mat.Color;
        rend.ColorMul = GFX::TGLColor(1.0, 1.0, 1.0, alpha);
        rend.TForm = tf;
