...
{
    FSMgr::FileHandle *v0 = FSMgr::iDir::openFileAlloc(uaDataFirstResolvedWritePath("env/ypa_log.txt"), "w");
    if ( v0 )
    {
...
void ypa_log_out(const char *format, ...)
{
    va_list va;
    va_start(va, format);
    FSMgr::FileHandle *f = FSMgr::iDir::openFileAlloc(uaDataFirstResolvedWritePath("env/ypa_log.txt"), "a");
    if ( f )
    {
        f->vprintf(format, va);
        delete f;
...
void init_d3dlog()
{
    FSMgr::FileHandle *v0 = FSMgr::iDir::openFileAlloc(uaDataFirstResolvedWritePath("env/d3dlog.txt"), "w");
    if ( v0 )
    {
...
void log_d3dlog(const char *format, ...)
{
    va_list va;
    va_start(va, format);
    FSMgr::FileHandle *f = FSMgr::iDir::openFileAlloc(uaDataFirstResolvedWritePath("env/d3dlog.txt"), "a");
    if ( f )
    {
        f->vprintf(format, va);
        delete f;
...
void init_dinputlog()
{
    FSMgr::FileHandle *v0 = FSMgr::iDir::openFileAlloc(uaDataFirstResolvedWritePath("env/dinplog.txt"), "w");
    if ( v0 )
    {
